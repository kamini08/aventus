...
import { CHARACTERISTIC_UUID, SERVICE_UUID } from "@/lib/constants/ble";
import { sendMessage, subscribeToMessages } from "@/network/ble";
import {
  Entypo,
  Feather,
  FontAwesome5,
...
const EmergencyAssistanceScreen: React.FC = () => {
  const { connectedDevice } = useLocalSearchParams();

  const [device, setDevice] = useState<Device>();
  const [messages, setMessages] = useState<string[]>(["Help!"]);

  useEffect(() => {
    if (connectedDevice && typeof connectedDevice === "string") {
      try {
        const parsedDevice = JSON.parse(connectedDevice);
        setDevice(parsedDevice);
      } catch (err) {
        console.error("Invalid device JSON", err);
      }
    }
  }, [connectedDevice]);
  ...
  useEffect(() => {
    ...
    subscribeToMessages(device, SERVICE_UUID, CHARACTERISTIC_UUID, (msg) => {
  ...
  const sendSignal = async () => {
    if (device) {
      await sendMessage(device, SERVICE_UUID, CHARACTERISTIC_UUID, "Help!");
    }
  };

  const askHelp = async () => {
    if (device) {
      await sendMessage(device, SERVICE_UUID, CHARACTERISTIC_UUID, `${messages}`);
    }
  };
